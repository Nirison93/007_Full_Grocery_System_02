            ...
            <div class="p-4 bg-white rounded-xl shadow-sm border border-gray-200">
              <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                ...
                <div>
                  <label class="block mb-2 text-sm font-medium text-gray-700"
                    >Purchase Price
                    <span v-if="form.sales_unit_id" class="text-green-600">
                      (per {{ getSalesUnitName(form.sales_unit_id) }})
                    </span>
                  </label
                  >
                  <input
                    v-model.number="purchasePricePerSalesUnitInput"
                    type="number"
                    step="0.01"
                    required
                    class="w-full px-4 py-2 text-gray-800 bg-white border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
...
const salesUnitsPerPurchaseUnit = computed(() => {
  const purchaseToTransferRate = Number(form.value.purchase_to_transfer_rate) || 0;
  const transferToSalesRate = Number(form.value.transfer_to_sales_rate) || 0;

  if (purchaseToTransferRate <= 0 || transferToSalesRate <= 0) {
    return 0;
  }

  return purchaseToTransferRate * transferToSalesRate;
});

const purchasePricePerSalesUnit = computed(() => {
  const purchasePrice = Number(form.value.purchase_price) || 0;
  const conversionRate = salesUnitsPerPurchaseUnit.value;

  if (purchasePrice <= 0 || conversionRate <= 0) {
    return "";
  }

  return (purchasePrice / conversionRate).toFixed(2);
});

const purchasePricePerSalesUnitInput = computed({
  get() {
    const converted = purchasePricePerSalesUnit.value;
    if (converted === "") {
  ...
  set(value) {
    ...
    const conversionRate = salesUnitsPerPurchaseUnit.value;
    const purchaseUnitPrice = conversionRate > 0
      ? numericValue * conversionRate
      : numericValue;

    form.value.purchase_price = Number(purchaseUnitPrice.toFixed(2));
  },
});

const purchasePriceFormulaText = computed(() => {
  const purchaseUnitName = getPurchaseUnitName(form.value.purchase_unit_id) || "Purchase Unit";
  const salesUnitName = getSalesUnitName(form.value.sales_unit_id) || "Sales Unit";
  const conversionRate = salesUnitsPerPurchaseUnit.value;

  if (conversionRate <= 0) {
    return "Set both conversion rates to auto-calculate from purchase unit";
  }

  if (!purchasePricePerSalesUnit.value) {
    return `1 ${purchaseUnitName} = ${conversionRate} ${salesUnitName}`;
  }

  return `1 ${purchaseUnitName} = ${conversionRate} ${salesUnitName} (auto-converted)`;
});
...
const handleSubmit = () => {
  ...
  // Prepare form data, excluding image when not replaced
  const submitData = { ...form.value, _method: "PUT" };
